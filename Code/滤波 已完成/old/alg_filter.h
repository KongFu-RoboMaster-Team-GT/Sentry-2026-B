...
class Class_Filter_XYZ_Kalman
{
    ...
    inline float Get_Out(float *__Out);
    ...
    float _P[3] = {0.0f, 0.0f, 0.0f};
		
		float Q[3] = {0.1f, 0.1f, 0.1f};
		float	R[3] = {1.0f, 1.0f, 1.0f}; // x y z
...
inline float Class_Filter_XYZ_Kalman::Get_Out(float *__Out)
{
    for (int i = 0; i < 3; i++)
    {
        __Out[i] = Out[i];
